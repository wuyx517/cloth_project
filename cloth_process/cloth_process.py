...
def init_cloth_status(cloth_image, draw_box_shape, affine_point, change_len):
    """
    ...
    return variety_matrix, affine_box_point_m, affine_box


def image_mix(cloth_image, draw_image, variety_matrix, affine_m, affine_box, change_len):
    ...
    for y in range(affine_box_shape[1]):
        for x in range(affine_box_shape[0]):
            ...
            if change_y_2 >= affine_box_shape[1] or change_x_2 >= affine_box_shape[0]:
                continue
...
if __name__ == '__main__':
    ...
    var_matrix, aff_box_point_m, aff_box = init_cloth_status(cloth, box_shape, af_point, ch_len)
    box_shape = box_shape[::-1]
    box_shape.append(3)
    draw = cv2.resize(draw, (200, 200))

    draw_ones = np.ones(box_shape) * 255
    draw_ones[150:350, 50:250, :] = draw

    # cv2.imwrite('t1.jpg', draw_ones)
    start_time = time.time()
    cloth_image = image_mix(cloth, draw_ones, var_matrix, aff_box_point_m, aff_box, ch_len)
    print('image mis time is : ', time.time() - start_time)
    cv2.imwrite('../output_blend/cloth_process_test.jpg', cloth_image)
